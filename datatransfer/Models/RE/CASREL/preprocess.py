...
from datatransfer.Models.RE import RE_utils, RE_settings
...
# 源数据路径
initial_dataset_path = '../../../../data/duie/'
...
def naive_char_to_token_span(span: Tuple[int, int], offset_mapping: List[Tuple[int, int]]):
    """
    简易的转换函数
    将character级别的span，转换为token级别的span
    *需要验证！*
    """
    token2origin = {}
    text_length = -1
...
def add_token_span_re_dataset(data_dicts: List[Dict[str, Any]]):
    """
    将关系数据集中的character级别index信息转换为token级别的index信息
    :param data_dicts:
